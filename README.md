...
| 左键拖动 | 移动窗口（位置自动记住） |
| 双击 | 立即刷新 |
| 右键 | 菜单：刷新 / 不透明度 / 开机自启 / 退出 |

**30 秒**刷新一次。同时只允许跑一份（多开会把额度接口打到限流）。

> `/api/oauth/usage` 的限额未公开。实测 12 次/小时长期无事，几秒内连发数次会撞
> 429；30 秒 = 120 次/小时，介于两者之间。敢这么定是因为退避本身就是限流器：
> 撞 429 后 90 秒起步指数退避、期间零请求，有效速率会被自动压回去。
> **如果你发现窗口经常挂着 `⟳`，就是这个值太激进了**——改
> `widget_qt.py` / `widget_tk.py` 里的 `REFRESH_SEC` 往回调。
...
指向哪个目标是自动判断的：跑 exe 时指向 exe 自己，跑源码时指向 `run_qt.vbs`
（直接跑 `.py` 会弹控制台）。注意 `dist\` 不在版本库里——新克隆的仓库要先
`build.ps1` 打包，否则启用时会提示找不到目标。
