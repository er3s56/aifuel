...
**Windows 任务栏模式（Qt / exe）**：右键勾选“显示在任务栏（托盘左侧）”。
全部额度项按两行横向排列，保留百分比、进度条、重置时间和失败状态，模型专属额度、
余额也照常显示。悬停可查看完整详情；取消勾选后回到原来的悬浮窗位置。选择会自动保存。
字段按文字宽度紧凑排列，百分比位数变化不改变列宽；背景透明、文字跟随系统任务栏的
浅色/深色主题。窗口通过 Windows owner 关系保持在任务栏之前，点击任务栏时不再
补偿置顶；仅位置、尺寸或 Explorer 窗口变化时更新布局，切回悬浮窗会恢复原有关系。
