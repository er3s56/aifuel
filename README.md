...
|---|---|
| 左键拖动 | 移动窗口（位置自动记住） |
| 拖动边缘或边角 | 调整窗口尺寸，字体大小保持不变（Qt / exe 版） |
| 双击 | 立即刷新 |
...
右键 → **布局** 可切换“纵向面板”和“横向紧凑条”。横向默认两列两行，拉宽后可排成一行，
缩窄后自动换行；额度、进度条、重置时间和异常状态都会保留，窗口不能缩到裁掉内容。
两种布局分别记住尺寸，右键可恢复当前布局的默认尺寸；锁定位置也会锁定尺寸和布局。
允许把窗口放到任务栏上，重启后保留该位置；这仍是独立悬浮窗，操作任务栏时可能被任务栏遮挡。
...
已有 CLI 凭证时优先使用 CLI，并尝试官方自动续期。CLI 无法提供有效授权时，
若默认配置中的账户与组织记录均匹配桌面端当前授权，则自动使用桌面端的有效令牌。
CLI 文件残留但令牌已清空也可恢复，无需删除文件或重新登录；桌面端授权更新后可提前结束授权等待。
显式设置 `CLAUDE_CONFIG_DIR`、缺少账户/组织记录或身份不匹配时，保留原有 CLI 授权来源。
